...
contract QuizApp{
    ...
    uint public constant REWARD_PER_SCORE= 0.01 ether; 
    ...
    modifier _onlyOwnerOrManager() {
        require(owner == msg.sender || gameManager == msg.sender, "Not owner.");
        _;
    }

 modifier _notPaused() {
        require(isQuizPaused == false, "Quiz paused");
        _;
    }
   
    function createQuiz(string memory _cid, string memory _title) public _onlyOwnerOrManager {
    ...
    function updateQuizDetails(uint _quizId,string memory _cid, string memory _title) public _onlyOwnerOrManager{
  ...
  // start quiz 
    function startQuiz(uint _quizId, uint _startTime,uint _endTime, uint _rewards) public payable _onlyOwnerOrManager{
        require(_quizId<quizId,"Not a valid ID");
        require(msg.sender == quizIdMapping[_quizId].creator,"Not owner");
        require(quizIdMapping[_quizId].endTime == 0,"Alread ended");
        require(msg.value == _rewards,"Rewards mismatch");
    ...
    function endQuiz(uint _quizId, uint8[QUIZ_LENGTH] memory _answers) public _onlyOwnerOrManager{
        require(_quizId<quizId,"Not a valid ID");
        require(msg.sender == quizIdMapping[_quizId].creator,"Not owner");
    ...
    function submitAnswers(uint _quizId, uint8[QUIZ_LENGTH] memory _answers) public{
        ...
        // add owner cannot submitAnswers
        require(quizIdMapping[_quizId].creator != msg.sender,"Creator cannot submit");
        //  cannot re submit
        require(userQuizSubmissionMapping[msg.sender][_quizId].quizId != quizId,"Cannot resubmit");
    ...
    function redeemRewards(uint _quizId) public _notPaused {
       ...
       (bool success, ) = payable(address(msg.sender)).call{
        ...
        emit RewardRedemption(msg.sender,_quizId,rewards);
    }
    ...
    function calculateScore(address _user,uint _quizId) public view returns (uint) {
        ...
        uint8[QUIZ_LENGTH] memory userAnswers = userQuizSubmissionMapping[_user][_quizId].answers; // quizSubmission[_user][_quizId];
        Quiz memory quiz = quizIdMapping[_quizId];
        uint8[QUIZ_LENGTH] memory actualAnswers = quiz.answers;
        uint score = 0;
       ...
       function calculateTotalScore(address _user) public view returns (uint) {
        ...
        for(uint i =0;i<userAttemptedQuiz.length;++i){
            ...
            uint8[QUIZ_LENGTH] memory userAnswers = userQuizSubmissionMapping[_user][quizId].answers;
            Quiz memory quiz = quizIdMapping[quizId];
            uint8[QUIZ_LENGTH] memory actualAnswers = quiz.answers;
            for(uint j =0;j<actualAnswers.length;++j){
    ...
    function calculateRewards(address _user,uint _quizId) public view returns (uint){
        ...
        uint rewards = userQuizSubmissionMapping[_user][quizId].redeemedRewards;
        uint calculatedRewards= userScore * QUIZ_REWARD_FACTOR * REWARD_PER_SCORE; // this can be calculated by using rewards/attemptedCount
        return calculatedRewards-rewards;
    }
    ...
    function getSubmittedAnswer(address _user,uint _quizId) public view returns (uint8[QUIZ_LENGTH] memory){
      uint8[QUIZ_LENGTH] storage answers= userQuizSubmissionMapping[_user][_quizId].answers;
      return answers;
    }

     function getSubmittedQuizAnswer(address _user,uint _quizId) public view returns (QuizSubmission memory){
      return userQuizSubmissionMapping[_user][_quizId];
    }
    ...
    function getUserAttemptedQuizIds(address _user)public view returns (uint[] memory){
        return userAttemptedQuizIds[_user];
    }
    
    receive() external payable {}

     function withdraw(uint256 _value) public payable _onlyOwner {
    ...
    // To add rewards
    function deposit() public payable _onlyOwnerOrManager {
        (bool success, ) = payable(address(this)).call{value: msg.value}("");
        require(success, "Fail to transfer rewards");
    }

    function pauseQuiz(bool _pauseQuiz) public _onlyOwnerOrManager {
        isQuizPaused = _pauseGame;
    }
}