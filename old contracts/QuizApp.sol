...
contract NewApp{
     ...
     uint public constant QUIZ_REWARD_FACTOR= 0.005 ether;
  ...
  // start quiz 
    ...
    function calculateScore(address _user,uint _quizId) public view returns (uint) {
        ...
        uint8[QUIZ_LENGTH] memory userAnswers = quizSubmission[_user][_quizId];
    ...
    function calculateRewards(address _user,uint _quizId) public view returns (uint){
        ...
        return userScore * QUIZ_REWARD_FACTOR;
    }
    ...
    function getSubmittedAnswer(address _user,uint _quizId) public view returns (uint8[QUIZ_LENGTH] memory){
      uint8[QUIZ_LENGTH] storage answers= quizSubmission[_user][_quizId];
      return answers;
    }
